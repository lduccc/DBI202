...
CREATE TABLE Course_Material (
	...
	N'Sách từ vựng', N'Tệp âm thanh', N'Video Links', N'Học liệu', N'Sách luyện đề', N'Tài liệu tham khảo'))
);
GO
...
LEFT JOIN Exam_Result g ON s.id = g.student_id
...
BEGIN
    ...
    SELECT @avg = AVG(er.value) FROM Exam_Result er
...
BEGIN
    ...
    PRINT N'Đã cập nhật số dư cho sinh viên ' + @StudentID + N'.';
END
GO

CREATE OR ALTER PROCEDURE usp_ProcessCoursePayment
    @PaymentID VARCHAR(5), 
    @StudentID VARCHAR(5), 
    @ClassID NVARCHAR(20),
...
BEGIN
    ...
    BEGIN
        PRINT N'Giao dịch thanh toán ' + @PaymentID + N' cho sinh viên ' + @StudentID + N' - Trạng thái: Failed. Lỗi: ClassID ''' + @ClassID + ''' không tồn tại.';
        RETURN
    END

    IF NOT EXISTS (SELECT 1 FROM Student WHERE id = @StudentID)
    BEGIN
        PRINT N'Giao dịch thanh toán ' + @PaymentID + N' cho sinh viên ' + @StudentID + N' - Trạng thái: Failed. Lỗi: StudentID ''' + @StudentID + ''' không tồn tại.';
        RETURN
    END
    ...
    DECLARE @FinalNotes NVARCHAR(MAX) = ISNULL(@TransactionNotes, N'');
...
AS
BEGIN
    SELECT S.id AS StudentID, S.last_name + N' ' + S.first_name AS StudentFullName, S.balance
    FROM Student S
...
CREATE PROCEDURE usp_GetCoursePaymentSummary @CourseID NVARCHAR(50)
AS
BEGIN
    SELECT C.description AS CourseName, COUNT(P.id) AS TotalPayments,
...
CREATE PROCEDURE usp_GetStudentsWithHighestTotalPayment @TopN INT = 5 
AS
BEGIN
    SELECT TOP (@TopN) S.id AS StudentID, S.last_name + N' ' + S.first_name AS StudentFullName, 
        ISNULL(SUM(P.amount), 0) AS TotalAmountPaid 
    FROM Student S
...
BEGIN
    SET NOCOUNT ON
    DELETE Payment FROM Payment p JOIN deleted d ON p.class_id = d.id
    DELETE Exam_Result FROM Exam_Result g JOIN Exam e ON g.exam_id = e.id JOIN deleted d ON e.class_id = d.id
    DELETE Exam FROM Exam e JOIN deleted d ON e.class_id = d.id
    DELETE Class_Student FROM Class_Student cs JOIN deleted d ON cs.class_id = d.id
    DELETE Class FROM Class c JOIN deleted d ON c.id = d.id
END
GO
